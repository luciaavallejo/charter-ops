...
const COLORS = [
  ...
  { id:"c11", hex:"#C4B5FD", name:"Lila Pastel"},
  { id:"c12", hex:"#9CA3AF", name:"Gray"       },
];

const BRIEFING_FIELDS = [
  "DISPO","OA","CONTRATO","PAX","CATERING",
  "TRIPU","BRIF","POP","COBRO","P1 / PNR",
...
function parseSmartTime(raw: string): string {
  ...
  if (!digits) return "";
  if (digits.length <= 2) { const h=parseInt(digits); if(h>=0&&h<=23) return `${String(h).padStart(2,"0")}:00`; return ""; }
  if (digits.length === 3) { const h=parseInt(digits[0]); const m=parseInt(digits.slice(1)); if(h>=0&&h<=9&&m>=0&&m<=59) return `0${h}:${String(m).padStart(2,"0")}`; }
  if (digits.length >= 4) { const h=parseInt(digits.slice(0,2)); const m=parseInt(digits.slice(2,4)); if(h>=0&&h<=23&&m>=0&&m<=59) return `${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`; }
  return "";
}
...
function parseSmartDate(raw: string): string {
  const digits = raw.replace(/\D/g,"");
  if (!digits) return "";
  const yp = Math.floor(new Date().getFullYear()/100);
  if (digits.length===6) { const d=digits.slice(0,2),m=digits.slice(2,4),y=digits.slice(4,6); return `${yp}${y}-${m}-${d}`; }
...
function inPeriod(f: any, period: string): boolean {
  const d = new Date(f.depDate); const now = new Date();
  if (period==="day") return f.depDate===todayStr();
  if (period==="week") { const wa=new Date(); wa.setDate(wa.getDate()-7); return d>=wa&&d<=now; }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,minWidth:100}}>
      <svg width={88} height={88} viewBox="0 0 88 88">
        ...
        <circle cx={cx} cy={cy} r={r} fill="none" stroke={color} strokeWidth={10} strokeDasharray={`${dash} ${circ}`} strokeDashoffset={circ/4} strokeLinecap="round"/>
...
function useT(theme: string) {
  ...
  return { dark, bg:dark?"#0A0E1A":"#F8FAFC", surface:dark?"#0F1629":"#FFFFFF", border:dark?"#1E293B":"#E2E8F0", text:dark?"#F1F5F9":"#0F172A", muted:dark?"#64748B":"#94A3B8", sub:dark?"#475569":"#94A3B8", inp:dark?"#1E293B":"#F1F5F9" };
}

function TimeInput({ value, onChange, placeholder }: { value:string,onChange:(v:string)=>void,placeholder?:string }) {
  const [raw,setRaw]=useState(value);
  useEffect(()=>setRaw(value),[value]);
  function handleBlur() { const p=parseSmartTime(raw); if(p){setRaw(p);onChange(p);}else if(!raw)onChange(""); }
  return <input value={raw} onChange={e=>setRaw(e.target.value)} onBlur={handleBlur} placeholder={placeholder||"HH:MM"} style={{width:"100%",padding:"9px 12px",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:8,color:"#0F172A",fontSize:13,outline:"none"}}/>;
}

function DateInput({ value, onChange, placeholder, T }: { value:string,onChange:(v:string)=>void,placeholder?:string,T?:any }) {
  const [raw,setRaw]=useState(value);
  useEffect(()=>setRaw(value),[value]);
  function handleBlur() { const p=parseSmartDate(raw); if(p){setRaw(p);onChange(p);}else if(raw&&raw.includes("-")){onChange(raw);}else if(!raw)onChange(""); }
  const bg=T?T.inp:"#F8FAFC"; const border=T?T.border:"#E2E8F0"; const color=T?T.text:"#0F172A";
  return (
    <div style={{display:"flex",gap:6}}>
      <input type="text" value={raw} onChange={e=>setRaw(e.target.value)} onBlur={handleBlur} placeholder={placeholder||"01012026"} style={{flex:1,padding:"9px 12px",background:bg,border:`1px solid ${border}`,borderRadius:8,color,fontSize:13,outline:"none"}}/>
      <input type="date" value={raw} onChange={e=>{setRaw(e.target.value);onChange(e.target.value);}} style={{padding:"9px 8px",border:`1px solid ${border}`,borderRadius:8,background:bg,color,fontSize:13,outline:"none",width:42,flexShrink:0}}/>
    </div>
  );
}

function AuthScreen({ onLogin }: { onLogin:(u:any,users:any)=>void }) {
  const [stored,setStored]=useState<Record<string,any>>(()=>{ try{return JSON.parse(localStorage.getItem("ch_users")||"{}")}catch{return{}} });
  ...
  function doRegister() {
    ...
    const next={...stored,[email]:{password:pass,color,name,isAdmin:isFirst||email==="luciaavallejo@gmail.com"}};
    localStorage.setItem("ch_users",JSON.stringify(next));
    setStored(next);
    onLogin({email,color,name,isAdmin:next[email].isAdmin},next);
  }
  ...
  function doLogin() {
    ...
    onLogin({email,color:u.color,name:u.name,isAdmin:u.isAdmin||email==="luciaavallejo@gmail.com"},stored);
  }
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif"}}>
      <style>{`*{box-sizing:border-box;margin:0;padding:0}button,input,select{font-family:inherit;outline:none}`}</style>
      <div style={{width:430,background:T.surface,border:`1px solid ${T.border}`,borderRadius:16,padding:"40px 36px",boxShadow:"0 8px 30px rgba(0,0,0,0.08)"}}>
        ...
        {mode==="register"&&(
          <div style={{marginBottom:14}}>
            <label style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:"0.08em"}}>Username</label>
            <input value={username} onChange={e=>setUsername(e.target.value)} placeholder="LUCIA" style={{...inp,marginTop:6}}/>
          </div>
        )}
        {mode==="login"&&(
...
function BriefingPanel({ flight, userColor, onStatusChange, onSendMessage, onAddReminder, T }: any) {
  ...
  function cycle(field:string){const cur=flight.briefing[field]||"gray",idx=STATUS_CYCLE.indexOf(cur);onStatusChange(flight.id,field,STATUS_CYCLE[(idx+1)%STATUS_CYCLE.length]);}
  function addEmail(){if(!rInput.trim()||rEmails.includes(rInput.trim()))return;const n=[...rEmails,rInput.trim()];setREmails(n);localStorage.setItem("ch_rem_emails",JSON.stringify(n));setRInput("");}
    ...
    <div style={{padding:"16px 20px 20px",background:T.dark?"#060A14":"#F8FAFC",borderTop:`1px solid ${T.border}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{flex:1}}>
          ...
          <div style={{height:4,background:T.border,borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,background:pct===100?"#22C55E":"#3B82F6",borderRadius:2,transition:"width 0.3s"}}/></div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6}}>
        {BRIEFING_FIELDS.map(field=>{
          ...
          return(
            <div key={field} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,padding:"7px 9px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <button onClick={()=>cycle(field)} style={{width:9,height:9,borderRadius:"50%",background:meta?.bg||"#9CA3AF",border:"none",cursor:"pointer",flexShrink:0,boxShadow:status!=="gray"&&status!=="black"?`0 0 4px ${meta?.bg}80`:"none"}}/>
                <span style={{fontSize:9,fontWeight:600,color:T.muted,letterSpacing:"0.02em"}}>{field}</span>
              </div>
              <button onClick={()=>{setOpenChat(openChat===field?null:field);onSendMessage(flight.id,field,null,true);}} style={{background:"none",border:"none",cursor:"pointer",padding:"1px 3px"}}>
                {total>0?<span style={{fontSize:9,fontWeight:700,color:unread>0?"#3B82F6":"#9CA3AF",background:T.inp,borderRadius:10,padding:"1px 4px",minWidth:14,display:"inline-block",textAlign:"center"}}>{total}</span>:<span style={{fontSize:10,color:T.dark?"#334155":"#CBD5E1"}}>💬</span>}
              </button>
            </div>
          );
        })}
...
function FlightModal({ flight, onClose, onSave, clients, setClients, T, title }: any) {
  const isEdit=!!flight;
  const [form,setForm]=useState({
    localizador:flight?.localizador||"", tripId:flight?.tripId||"",
    origin:flight?.origin||"", destination:flight?.destination||"",
    depDate:flight?.depDate||"", depTime:flight?.depTime||"",
    arrDate:flight?.arrDate||"", arrTime:flight?.arrTime||"", type:flight?.type||"charter"
  });
  ...
  function save(){
    ...
    if(isEdit){
      ...
      onSave({...form,client:clientVal,plane:planeVal,id:"FL"+Date.now().toString(36).toUpperCase(),responsibles:[],archived:false,briefing:initBriefing(),messages:{},reminders:[],updatedAt:Date.now()});
    }
    onClose();
  }
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000}}>
      <div style={{width:560,background:T.surface,border:`1px solid ${T.border}`,borderRadius:16,padding:"28px 28px 24px",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div><label style={lbl}>Departure Date * (ej: 01012026)</label><DateInput value={form.depDate} onChange={v=>set("depDate",v)} placeholder="01012026" T={T}/></div>
          <div><label style={lbl}>Departure Time * (ej: 1000)</label><TimeInput value={form.depTime} onChange={v=>set("depTime",v)} placeholder="1000 → 10:00"/></div>
          <div><label style={lbl}>Arrival Date</label><DateInput value={form.arrDate} onChange={v=>set("arrDate",v)} placeholder="01012026" T={T}/></div>
          ...
          <div style={{gridColumn:"span 2",position:"relative"}}>
            ...
            {cSugs.length>0&&clientVal&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,zIndex:10,marginTop:3}}>
              {cSugs.map((c:string)=>(
                <div key={c} style={{padding:"9px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${T.border}`}} onMouseOver={e=>(e.currentTarget as HTMLDivElement).style.background=T.inp} onMouseOut={e=>(e.currentTarget as HTMLDivElement).style.background="transparent"}>
                  <span onClick={()=>{setClientVal(c);setCSugs([]);}} style={{cursor:"pointer",color:T.text,fontSize:13,flex:1}}>{c}</span>
                  <button onClick={e=>{e.stopPropagation();const nc=clients.filter((x:string)=>x!==c);setClients(nc);localStorage.setItem("ch_clients",JSON.stringify(nc));setCSugs(nc.filter((x:string)=>x.toLowerCase().includes(clientVal.toLowerCase())));}} style={{background:"none",border:"none",color:"#EF4444",cursor:"pointer",fontSize:12,padding:"0 4px"}}>✕</button>
                </div>
              ))}
            </div>}
          </div>
        ...
        <div style={{marginTop:22,display:"flex",gap:10,justifyContent:"flex-end"}}>
          ...
          <button onClick={save} style={{padding:"10px 20px",background:"#0F172A",border:"none",borderRadius:8,color:"#fff",fontWeight:700,cursor:"pointer",fontSize:13}}>{isEdit?"Save Changes ✓":"Create Flight ✈️"}</button>
        </div>
      </div>
    </div>
  );
...
function SettingsPanel({ theme, setTheme, onClose, T, users, setUsers, currentUser }: any) {
  ...
  function deleteUser(email:string){if(email===currentUser.email)return;if(!confirm(`Delete user ${email}?`))return;const next={...users};delete next[email];localStorage.setItem("ch_users",JSON.stringify(next));setUsers(next);}
  function changeColor(email:string,colorId:string){const taken=Object.entries(users).filter(([e])=>e!==email).map(([,u]:any)=>u.color);if(taken.includes(colorId))return alert("Color already taken");const next={...users,[email]:{...users[email],color:colorId}};localStorage.setItem("ch_users",JSON.stringify(next));setUsers(next);}
  const isAdmin=currentUser.isAdmin||currentUser.email==="luciaavallejo@gmail.com";
  return(
    <div style={{position:"fixed",inset:0,zIndex:3000,display:"flex",justifyContent:"flex-end"}}>
      ...
      <div style={{width:320,background:T.surface,borderLeft:`1px solid ${T.border}`,padding:24,display:"flex",flexDirection:"column",gap:20,overflowY:"auto"}}>
        ...
        {isAdmin&&(
          <div>
            ...
            {userList.map((u:any)=>{
              ...
              const taken=Object.entries(users).filter(([e])=>e!==u.email).map(([,uu]:any)=>uu.color);
              const avail=COLORS.filter(c=>!taken.includes(c.id));
              return(
        ...
        <div style={{padding:14,background:T.inp,borderRadius:10}}>
          ...
          <div style={{fontSize:13,color:T.text,fontWeight:600}}>Charter Ops v9.0</div>
...
function StatsModule({ flights, users, T, onGoTo }: any) {
  ...
  const perPerson=userList.map((u:any)=>{const myF=active.filter((f:any)=>f.responsibles.includes(u.color));const col=COLORS.find(c=>c.id===u.color);return{name:u.name,color:col?.hex||"#94A3B8",count:myF.length};}).filter((x:any)=>x.count>0);
  const clientMap:Record<string,number>={};active.forEach((f:any)=>{if(f.client)clientMap[f.client]=(clientMap[f.client]||0)+1;});
  ...
  const planeMap:Record<string,number>={};active.forEach((f:any)=>{if(f.plane)planeMap[f.plane]=(planeMap[f.plane]||0)+1;});
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        {[{label:"Total Flights",value:total,color:"#3B82F6"},{label:"Completed",value:done,color:"#22C55E",click:()=>onGoTo("green")},{label:"Critical",value:critical,color:"#EF4444",click:()=>onGoTo("red")},{label:"In Progress",value:active.filter((f:any)=>getStatus(f.briefing)==="yellow").length,color:"#EAB308",click:()=>onGoTo("yellow")}].map(({label,value,color,click}:any)=>(
          <div key={label} onClick={click} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px",cursor:click?"pointer":"default",borderTop:`3px solid ${color}`}}>
            <div style={{fontSize:28,fontWeight:700,color}}>{value}</div>
            <div style={{fontSize:11,color:T.muted,marginTop:3}}>{label} · {periodLabel}</div>
          </div>
        ))}
      </div>
      {card(<>{sec(`👤 Per Person — ${periodLabel}`)}{perPerson.length===0?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:16}}>No data</div>:<div style={{display:"flex",gap:20,flexWrap:"wrap"}}>{perPerson.map((u:any)=><DonutChart key={u.name} value={u.count} total={total||1} color={u.color} label={u.name} sublabel={`${u.count} flight${u.count!==1?"s":""}`}/>)}</div>}</>)}
      {card(<>{sec(`🏢 Per Client — ${periodLabel}`)}{perClient.length===0?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:16}}>No data</div>:<div style={{display:"flex",gap:20,flexWrap:"wrap"}}>{perClient.map((c:any)=><DonutChart key={c.name} value={c.count} total={total||1} color={c.color} label={c.name} sublabel={`${c.count} flight${c.count!==1?"s":""}`}/>)}</div>}</>)}
      {card(<>{sec(`✈️ Per Aircraft — ${periodLabel}`)}{perPlane.length===0?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:16}}>No data</div>:<div style={{display:"flex",gap:16,flexWrap:"wrap"}}>{perPlane.map((p:any)=><DonutChart key={p.name} value={p.count} total={total||1} color={p.color} label={p.name} sublabel={`${p.count} flight${p.count!==1?"s":""}`}/>)}</div>}</>)}
      {card(<>{sec("📊 Briefing Completeness")}{allActive.map((f:any)=>{const pct=getPct(f.briefing),s=getStatus(f.briefing),sm=STATUS_META[s];return(<div key={f.id} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:T.text}}>{f.origin} → {f.destination} <span style={{color:T.muted}}>({f.localizador})</span></span><span style={{fontSize:12,fontWeight:700,color:sm?.bg}}>{pct}%</span></div><div style={{height:4,background:T.border,borderRadius:3}}><div style={{height:"100%",width:`${pct}%`,background:sm?.bg,borderRadius:3}}/></div></div>);})}</>)}
    </div>
  );
}

export default function App() {
  const [theme,setTheme]=useState<string>(()=>{ if(typeof window!=="undefined")return localStorage.getItem("ch_theme")||"light"; return "light"; });
  const [user,setUser]=useState<any>(()=>{ if(typeof window==="undefined")return null; try{return JSON.parse(sessionStorage.getItem("ch_session")||"null");}catch{return null;} });
  const [users,setUsers]=useState<Record<string,any>>(()=>{ if(typeof window==="undefined")return {}; try{return JSON.parse(localStorage.getItem("ch_users")||"{}");}catch{return{};} });
  const [clients,setClients]=useState<string[]>(()=>{ if(typeof window==="undefined")return["ACS España","LunaJets"]; try{return JSON.parse(localStorage.getItem("ch_clients")||'["ACS España","LunaJets"]');}catch{return["ACS España","LunaJets"];} });
  const t0=todayStr(),t1=tomorrowStr(),t2=weekEndStr();
  const [flights,setFlights]=useState<any[]>([
    {id:"FL001",localizador:"ABC123",tripId:"AV-001",origin:"LEMD",destination:"EGLL",depDate:t0,depTime:"09:00",arrDate:t0,arrTime:"11:30",type:"charter",client:"ACS España",plane:"EC-MUS",responsibles:[],archived:false,briefing:{...initBriefing(),DISPO:"green",OA:"green",CONTRATO:"yellow",PAX:"red",CATERING:"red"},messages:{PAX:[{user:"c4",text:"Waiting for PAX list",date:t0,time:"10:23",read:true}]},reminders:[],updatedAt:Date.now()-1800000},
    {id:"FL002",localizador:"XYZ789",tripId:"",origin:"LEBL",destination:"LFPG",depDate:t1,depTime:"14:30",arrDate:t1,arrTime:"16:45",type:"empty leg",client:"LunaJets",plane:"EC-LYL",responsibles:[],archived:false,briefing:{...initBriefing(),DISPO:"green",OA:"green",CONTRATO:"green",PAX:"green",BRIF:"yellow"},messages:{},reminders:[],updatedAt:Date.now()-3600000},
    {id:"FL003",localizador:"DEF456",tripId:"AV-002",origin:"LEMD",destination:"LIRF",depDate:t2,depTime:"07:15",arrDate:t2,arrTime:"09:30",type:"charter",client:"ACS España",plane:"EC-MRL",responsibles:[],archived:false,briefing:Object.fromEntries(BRIEFING_FIELDS.map(f=>[f,"green"])),messages:{},reminders:[],updatedAt:Date.now()-7200000},
  ]);
  const [expanded,setExpanded]=useState<string|null>(null);
  const [flightModal,setFlightModal]=useState<any>(null); // null=closed, "new"=new, flight=edit
  ...
  function push(icon:string,title:string,body:string){setNotifs(p=>[{icon,title,body,time:new Date().toLocaleTimeString("es-ES"),read:false},...p].slice(0,50));}

  function handleLogin(u:any,freshUsers:any){setUser(u);if(freshUsers)setUsers(freshUsers);sessionStorage.setItem("ch_session",JSON.stringify(u));}
  function handleSignOut(){sessionStorage.removeItem("ch_session");setUser(null);}
  ...
  function handleStatus(fid:string,field:string,status:string){setFlights(fs=>fs.map(f=>{if(f.id!==fid)return f;const b={...f.briefing,[field]:status};if(field==="CREDIT HOLD"&&status==="green")b["COBRO"]="green";push("🔔",`${field} updated`,`${f.localizador}: ${field} → ${status}`);return{...f,briefing:b,updatedAt:Date.now()};}));}
  function handleMsg(fid:string,field:string,msgText:string|null,markRead:boolean){setFlights(fs=>fs.map(f=>{if(f.id!==fid)return f;const msgs={...f.messages};if(markRead){msgs[field]=(msgs[field]||[]).map((m:any)=>({...m,read:true}));}else{const now=new Date();msgs[field]=[...(msgs[field]||[]),{user:user.color,text:msgText,date:now.toISOString().slice(0,10),time:now.toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"}),read:true}];push("💬",`New message in ${field}`,`${f.localizador}: "${(msgText||"").slice(0,40)}"`);}return{...f,messages:msgs,updatedAt:Date.now()};}));}
  function handleSaveFlight(flight:any){
    if(flightModal==="new"){setFlights(f=>[flight,...f]);push("✈️","New flight created",`${flight.origin} → ${flight.destination} (${flight.localizador})`);}
    else{setFlights(fs=>fs.map(f=>f.id===flight.id?flight:f));push("✏️","Flight updated",`${flight.localizador} edited`);}
  }
  function handleArchive(id:string,e:any){e.stopPropagation();setFlights(fs=>fs.map(f=>f.id===id?{...f,archived:!f.archived}:f));}
  function handleReminder(fid:string,rem:any){setFlights(fs=>fs.map(f=>f.id===fid?{...f,reminders:[...(f.reminders||[]),rem]}:f));push("📅","Reminder set",`${rem.date} ${rem.time}${rem.note?` · ${rem.note}`:""}`);}
  function handleGoTo(type:string,id?:string){setActiveTab("dashboard");setFClient("all");setFColor("all");setFDate("all");setMyOnly(false);setShowArch(false);setSearch("");setFStatus("all");if(type==="red")setFStatus("red");if(type==="green")setFStatus("green");if(type==="yellow")setFStatus("yellow");if(type==="id"&&id)setSearch(id);}
  function handleDashboardClick(){setActiveTab("dashboard");setFStatus("all");setFClient("all");setFColor("all");setFDate("all");setMyOnly(false);setShowArch(false);setSearch("");setExpanded(null);}

  const active=flights.filter(f=>!f.archived),archived=flights.filter(f=>f.archived),pool=showArch?archived:active;
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"system-ui,sans-serif",color:T.text}}>
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:56,position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:24}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:18}}>✈️</span><span style={{fontSize:17,fontWeight:700,color:T.text}}>Charter</span></div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={()=>setShowNotif(v=>!v)} style={{position:"relative",background:"none",border:`1px solid ${T.border}`,borderRadius:8,padding:"5px 10px",color:T.muted,cursor:"pointer",fontSize:16}}>
            🔔{unreadMsgs>0&&<span style={{position:"absolute",top:-4,right:-4,background:"#EF4444",color:"#fff",fontSize:9,fontWeight:700,borderRadius:10,padding:"1px 5px",minWidth:16,textAlign:"center"}}>{unreadMsgs}</span>}
          </button>
      ...
      <div style={{padding:"20px 24px"}}>
        {activeTab==="dashboard"&&(<>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
            <button onClick={()=>setFlightModal("new")} style={{padding:"9px 16px",background:"#0F172A",border:"none",borderRadius:8,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>+ New Flight</button>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search LOC / ICAO / Trip ID..." style={{padding:"9px 14px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13,width:260}}/>
                      ...
                      <td style={{padding:"13px 14px"}}>
                        <div style={{display:"flex",gap:5}}>
                          <button onClick={e=>{
                            ...
                            const userColorId=user.color;
                            setFlights(prev=>{
                              return prev.map(f=>{
                                if(f.id!==flightId)return f;
                                const alreadyIn=f.responsibles.includes(userColorId);
                                const newResp=alreadyIn
                                  ?f.responsibles.filter((r:string)=>r!==userColorId)
                                  :[...f.responsibles,userColorId];
                          ...
                          <button onClick={e=>{e.stopPropagation();setFlightModal(flight);}} style={{background:T.inp,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 8px",color:T.muted,fontSize:11,cursor:"pointer"}} title="Edit flight">✏️</button>
                        </div>
                      </td>
                    </tr>,
      ...
      {flightModal&&<FlightModal flight={flightModal==="new"?null:flightModal} onClose={()=>setFlightModal(null)} onSave={handleSaveFlight} clients={clients} setClients={setClients} T={T} title={flightModal==="new"?"+ New Flight":`✏️ Edit — ${flightModal.localizador}`}/>}
...
}// v9-viernes, 24 de abril de 2026, 10:46:07 CEST
