...
const BRIEFING_FIELDS = [
  "DISPO","OA","CONTRATO","PAX","CAT","BRIF",
  "POP","COBRO","FACTURA F.","TRIPU","P1 / PNR",
  "SLOTS","CATERING","CREDIT HOLD","MOVIMIENTO"
];
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000}}>
      <div style={{width:560,background:T.surface,border:`1px solid ${T.border}`,borderRadius:16,padding:"28px 28px 24px",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div><label style={lbl}>Departure Date *</label><input type="date" style={inp} value={form.depDate} onChange={e=>set("depDate",e.target.value)}/></div>
          ...
          <div><label style={lbl}>Arrival Date</label><input type="date" style={inp} value={form.arrDate} onChange={e=>set("arrDate",e.target.value)}/></div>
          ...
          <div style={{gridColumn:"span 2",position:"relative"}}>
            ...
            {cSugs.length>0&&clientVal&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,zIndex:10,marginTop:3}}>
              {cSugs.map((c:string)=><div key={c} onClick={()=>{setClientVal(c);setCSugs([]);}} style={{padding:"9px 12px",cursor:"pointer",color:T.text,fontSize:13,borderBottom:`1px solid ${T.border}`}} onMouseOver={e=>(e.currentTarget as HTMLDivElement).style.background=T.inp} onMouseOut={e=>(e.currentTarget as HTMLDivElement).style.background="transparent"}>{c}</div>)}
            </div>}
          </div>
          <div style={{gridColumn:"span 2",position:"relative"}}>
...
export default function App() {
  ...
  const bell = notifs.filter(n=>!n.read).length + active.reduce((s,f)=>s+Object.values(f.messages||{}).reduce((a:number,ms:any)=>a+ms.filter((m:any)=>!m.read).length,0),0);
      ...
      <div style={{padding:"20px 24px"}}>
        ...
        {activeTab==="dashboard"&&(<>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search LOC, reg, LEMD, Madrid..." style={{padding:"9px 14px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,color:T.text,fontSize:13,width:260}}/>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead>
                <tr style={{background:T.dark?"#080C18":"#F8FAFC"}}>
                  {["Team","Route","Salida (España)","Client","Status","Aircraft","Actions"].map(h=>(
                  ...
                  return[
                    <tr key={flight.id} onClick={()=>setExpanded(isExp?null:flight.id)} style={{borderBottom:`1px solid ${T.border}`,cursor:"pointer"}} onMouseOver={e=>(e.currentTarget as HTMLTableRowElement).style.background=T.dark?"#0F1629":"#F8FAFC"} onMouseOut={e=>(e.currentTarget as HTMLTableRowElement).style.background="transparent"}>
                      ...
                      <td style={{padding:"13px 14px"}}>
                        ...
                        <div style={{fontSize:12,color:T.sub}}>{localTime} (España)</div>
                      </td>
                      ...
                      <td style={{padding:"13px 14px"}}>
                        <div style={{display:"flex",gap:5}}>
                          <button onClick={e=>{e.stopPropagation();setFlights(fs=>fs.map(f=>{if(f.id!==flight.id)return f;const has=f.responsibles.includes(user.color);return{...f,responsibles:has?f.responsibles.filter((r:string)=>r!==user.color):[...f.responsibles,user.color],updatedAt:Date.now()};}));}} style={{background:isMine?"rgba(59,130,246,0.1)":T.inp,border:`1px solid ${isMine?"#3B82F6":T.border}`,borderRadius:6,padding:"4px 9px",color:isMine?"#3B82F6":T.muted,fontSize:11,fontWeight:600,cursor:"pointer"}}>{isMine?"✓ Mine":"+ Join"}</button>
